	...
	$.ajax({
		...
		success : function(data) {
			...
			for (var i = 0; i < parsed.length; i++) {
				str = addPost(parsed[i].mnm, parsed[i].bdrgdate,
						parsed[i].bdcontent, parsed[i].likehits,
						parsed[i].rplyhits, str);
			}
			...
			/*
			 * var data = returnData.rows; var html = "";
			 *  // 페이지가 1일 경우에만 id가 post-list인 html을 비운다. if (page == 1) {
			 * $("#post-list").html(""); }
			 * 
			 * if (returnData.startNum <= returnData.totCnt) { if (data.length >
			 * 0) { // for문을 돌면서 행을 그린다. } else { // 데이터가 없을 경우 } }
			 * 
			 * html = html.replace(/%20/gi, "");
			 * 
			 * if (page == 1) { // 페이지가 1이 아닐 경우 데이터를 붙인다.
			 * $("#post-list").html(html); }
			 */
		},
		error : function(e) {
...
function addPost(mnm, bdrgdate, bdcontent, likehits, rplyhits, str) {
	str += "<div class='card-post' id='post-list'>"
			...
			+ "</div>" + "<div class='col-xs-9 col-sm-10 info-user'>" + "<h3>"
